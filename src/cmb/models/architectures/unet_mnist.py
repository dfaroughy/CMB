...
from DynGenModels.models.architectures.utils import (
...
class Unet28x28(nn.Module):
    ...
    def Embeddings(self):
        self.projection = nn.Conv2d(
            1, self.dim_hidden, kernel_size=3, stride=1, padding=1
        )
        self.time_embedding = Time_embedding(
            self.dim_time_emb, self.dim_hidden, nn.GELU()
        )
    ...
    def Encoder(self):
        ...
        self.down3 = Down(
            in_channels=2 * self.dim_hidden,
            out_channels=4 * self.dim_hidden,
            time_channels=self.dim_hidden,
            activation=self.act,
            dropout=self.dropout,
        )
    ...
    def Decoder(self):
        self.up0 = nn.Sequential(
            nn.ConvTranspose2d(4 * self.dim_hidden, 4 * self.dim_hidden, 3, 3),
            nn.GroupNorm(8, 4 * self.dim_hidden),
            self.act,
        )

        self.up1 = Up(
            in_channels=8 * self.dim_hidden,
            out_channels=self.dim_hidden,
            time_channels=self.dim_hidden,
            activation=self.act,
            dropout=self.dropout,
        ...
        self.output = nn.Sequential(
            nn.Conv2d(
                2 * self.dim_hidden, self.dim_hidden, kernel_size=3, stride=1, padding=1
            ),
...
class Down(nn.Module):
    def __init__(self, in_channels, out_channels, time_channels, activation, dropout):
        ...
        self.conv_block = TemporalResidualConvBlock(
...
class Up(nn.Module):
    def __init__(self, in_channels, out_channels, time_channels, activation, dropout):
        ...
        self.conv_block1 = TemporalResidualConvBlock(
        ...
        self.conv_block2 = TemporalResidualConvBlock(
            in_channels=out_channels,
            out_channels=out_channels,
            time_channels=time_channels,
            activation=activation,
...
class Time_embedding(nn.Module):
    def __init__(self, dim_time_emb, dim_hidden, activation_fn=nn.GELU()):
        ...
        layers = [
            nn.Linear(dim_time_emb, dim_hidden),
            activation_fn,
    ...
    def forward(self, t):
        temb = transformer_timestep_embedding(
            t.squeeze(), self.dim_time_emb, max_positions=10000
        )
        return self.fc(temb)
