...
class KolmogorovArnoldNetwork(nn.Module):
    ...
    def define_deep_models(self, config):
        ...
        if hasattr(config, "time_embedding"):
            if config.time_embedding == "SinusoidalPositionalEncoding":
                self.time_embedding = SinusoidalPositionalEncoding(
                    self.dim_time_emb, max_period=10000
                )
            elif config.time_embedding == "GaussianFourierFeatures":
                self.time_embedding = GaussianFourierFeatures(
                    self.dim_time_emb, scale=0.5
                )
            elif config.time_embedding == "KANLinear":
                self.time_embedding = KANLinear(1, self.dim_time_emb)
            elif config.time_embedding == "Linear":
                self.time_embedding = nn.Linear(1, self.dim_time_emb)
            else:
        ...
        if self.dim_discrete > 0:
            ...
            self.discrete_embedding = nn.Embedding(
                config.vocab_size, self.dim_discrete_emb
            )
        else:
        ...
        if hasattr(config, "context_embedding") and self.dim_context > 0:
            ...
            if config.context_embedding == "Embedding":
                self.context_embedding = nn.Embedding(
                    config.vocab_size, self.dim_context_emb
                )
            elif config.context_embedding == "Linear":
                self.time_embedding = nn.Linear(self.dim_context, self.dim_context_emb)
            else:
                raise NotImplementedError
        else:
        ...
        self.layers = fc_block(
            dim_input=self.dim_continuous
            + self.dim_context_emb
            + self.dim_discrete_emb
            + self.dim_time_emb,
    ...
    def forward(self, t, x=None, k=None, context=None, mask=None):
        ...
        discrete = discrete.reshape(
            k.size(0), self.dim_input, self.vocab_size
        )  # rate logits
...
class KolmogorovArnoldNetwork(nn.Module):
    ...
    def define_deep_models(self, config):
        ...
        if config.time_embedding == "SinusoidalPositionalEncoding":
            self.time_embedding = nn.Sequential(
                SinusoidalPositionalEncoding(self.dim_time_emb, max_period=10000),
                nn.Linear(self.dim_time_emb, self.dim_time_emb),
            )
        elif config.time_embedding == "GaussianFourierFeatures":
            self.time_embedding = nn.Sequential(
                GaussianFourierFeatures(self.dim_time_emb, scale=0.5),
                nn.Linear(self.dim_time_emb, self.dim_time_emb),
            )
        elif config.time_embedding == "KANLinear":
            self.time_embedding = nn.Sequential(
                KANLinear(1, self.dim_time_emb),
                nn.Linear(self.dim_time_emb, self.dim_time_emb),
            )
        elif config.time_embedding == "Linear":
            self.time_embedding = nn.Linear(1, self.dim_time_emb)
        ...
        self.kan = kan_block(
            dim_input=self.dim_input + self.dim_context + self.dim_time_emb,
    ...
    def forward(self, t, x, k=None, context=None):
        ...
        h = (
            torch.concat([x, context, t_emb], dim=1)
            if context is not None
            else torch.concat([x, t_emb], dim=1)
        )
        return self.kan(h)
