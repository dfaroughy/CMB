...
class GenerativeDynamicsModule:
    ...
    def __init__(self, config: Union[Configs, str] = None, dataclass=None, device=None):
        if isinstance(config, str):
            config = Configs(config)  # get configs form yaml
            if device is not None:
                config.train.device = device  # override config device if provided

        self.config = config
        self.dataclass = dataclass
        self.dynamics = dynamics.get(self.config.dynamics.name)(
            self.config
        )  # get generative dynamical model from config
        self.model = models.get(self.config.model.name)(
            self.config
        )  # get NN from configs
        self.model = self.model.to(torch.device(self.config.train.device))
        self.workdir = (
            Path(self.config.experiment.workdir)
            / Path(self.config.experiment.run_name)
            / Path(self.config.experiment.name)
        )
    ...
    def train(self):
        ...
        optimizer = optimizers.get(self.config.train.optimizer.name)(
            self.model.parameters(), **self.config.train.optimizer.params.to_dict()
        )
        scheduler = schedulers.get(self.config.train.scheduler.name)(
            optimizer=optimizer, **self.config.train.scheduler.params.to_dict()
        )
        early_stopping = (
            self.config.train.epochs
            if self.config.train.early_stopping is None
            else self.config.train.early_stopping
        )
        min_epochs = (
            0 if self.config.train.min_epochs is None else self.config.train.min_epochs
        )
        print_epochs = (
            1
            if self.config.train.print_epochs is None
            else self.config.train.print_epochs
        )
        ...
        if hasattr(self.config.data, "preprocess"):
            ...
            print(
                "    - continuous data: {}".format(
                    self.config.data.preprocess.continuous
                )
            )
            print(
                "    - discrete data: {}".format(self.config.data.preprocess.discrete)
            )

            self.dataclass.source.preprocess(
                output_continuous=self.config.data.preprocess.continuous,
                output_discrete=self.config.data.preprocess.discrete,
            )

            self.dataclass.target.preprocess(
                output_continuous=self.config.data.preprocess.continuous,
                output_discrete=self.config.data.preprocess.discrete,
            )
        ...
        self.logger.logfile_and_console(
            "INFO: number of training parameters: {}".format(
        ...
        for epoch in tqdm(range(self.config.train.epochs), desc="epochs"):
            ...
            TERMINATE, IMPROVED = valid.checkpoint(
                min_epochs=min_epochs, early_stopping=early_stopping
            )
            ...
            if TERMINATE:
                stop_message = "early stopping triggered! Reached maximum patience at {} epochs".format(
                    epoch
                )
        ...
        self._save_last_epoch_ckpt()
        self._save_best_epoch_ckpt(
            not bool(dataloader.valid)
        )  # best = last epoch if there is no validation, needed as a placeholder for pipeline
    ...
    def load(self, checkpoint: str = "best"):
        if checkpoint == "best":
            print(
                "INFO: loading `best` epoch checkpoint on {} from:".format(
                    self.config.train.device
                )
            )
            print("  - {}".format(self.workdir / "best_epoch.ckpt"))
            self.best_epoch_ckpt = type(self.model)(self.config)
            self.best_epoch_ckpt.load_state_dict(
                torch.load(
                    self.workdir / "best_epoch.ckpt",
                    map_location=(
                        torch.device("cpu")
            ...
            print(
                "INFO: loading `last` epoch checkpoint on {} from:".format(
                    self.config.train.device
                )
            )
            print("  - {}".format(self.workdir / "last_epoch.ckpt"))
            self.last_epoch_ckpt = type(self.model)(self.config)
            self.last_epoch_ckpt.load_state_dict(
                torch.load(
                    self.workdir / "last_epoch.ckpt",
                    map_location=(
                        torch.device("cpu")
                        if self.config.train.device == "cpu"
                        else None
            ...
            print(
                "INFO: loading `{}` checkpoint on {} from:".format(
                    checkpoint, self.config.train.device
                )
            )
            ...
            self.checkpoint.load_state_dict(
                torch.load(
                    self.workdir / f"{checkpoint}.ckpt",
                    map_location=(
                        torch.device("cpu")
                        if self.config.train.device == "cpu"
                        else None
    ...
    def generate(self, dataclass=None, output_history=False, save_to=None, **kwargs):
        ...
        if hasattr(self, "best_epoch_ckpt"):
            model = self.best_epoch_ckpt
        elif hasattr(self, "last_epoch_ckpt"):
            model = self.last_epoch_ckpt
        else:
        ...
        out_continuous, out_discrete = self.dynamics.solver.simulate(
            model, time_steps=time_steps, output_history=output_history, **kwargs
        )
        if out_continuous is not None and out_discrete is None:
            ...
            sample = (
                torch.cat([out_continuous[-1], out_discrete[-1]], dim=-1)
                if output_history
                else torch.cat([out_continuous, out_discrete], dim=-1)
            )
            self.trajectories = out_continuous if output_history else None
            self.jumps = out_discrete if output_history else None
        else:
            raise ValueError(
                "Both trajectories and jumps cannot be `None` simultaneously."
            )
        ...
        if dataclass is not None:
            ...
            self.sample.postprocess(
                input_continuous=self.config.data.preprocess.continuous,
    ...
    def _log_losses(self, train, valid, epoch, print_epochs=1):
        message = (
            "\tEpoch: {}, train loss: {}, valid loss: {}  (min valid loss: {})".format(
                epoch, train.loss, valid.loss, valid.loss_min
            )
        )
