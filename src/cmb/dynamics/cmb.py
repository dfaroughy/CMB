...
class ConditionalMarkovBridge:
    ...
    def __init__(self, config: dataclass):
        ...
        if hasattr(config.dynamics, "continuous"):
            self.process_continuous = processes["continuous"].get(
                config.dynamics.continuous.process
            )(config)
            self.loss_continuous_fn = MSELoss(reduction="none")

        if hasattr(config.dynamics, "discrete"):
            self.process_discrete = processes["discrete"].get(
                config.dynamics.discrete.process
            )(config)
            self.loss_discrete_fn = CrossEntropyLoss(reduction="none")
            self.weight = (
                config.dynamics.loss_weight
                if hasattr(config.dynamics, "loss_weight")
                else 1.0
            )
    ...
    def sample_coupling(self, batch):
        ...
        # ...mask and context:
        self.mask = getattr(
            batch, "target_mask", torch.ones_like(self.x0[..., 0]).unsqueeze(-1)
        )
    ...
    def sample_bridges(self):
        ...
        self.xt = (
            self.process_continuous.sample(t=self.t, x0=self.x0, x1=self.x1)
            if hasattr(self, "process_continuous")
            else None
        )
        self.kt = (
            self.process_discrete.sample(t=self.t, k0=self.k0, k1=self.k1)
            if hasattr(self, "process_discrete")
            else None
        )
    ...
    def loss(self, model, batch):
        ...
        if hasattr(self, "process_continuous"):
            ut = self.process_continuous.drift(
                t=self.t, x=self.xt, x0=self.x0, x1=self.x1
            ).to(vector.device)
