...
def animate_trajectories(paths, jumps, title='CMB', N=2000, cmap='tab10', show_paths=True, filename='trajectories.gif', fps=10):
    """
    ...
    def update(frame):
        ...
        if show_paths:
            for i in range(N):
                # Update trajectories
                lines[i].set_data(paths[:idx_path+1, i, 0], paths[:idx_path+1, i, 1])
