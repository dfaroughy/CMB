...
class JetDataclass:
    ...
    def __init__(self, config, task, num_jets=None):
        assert (
            task == "train" or task == "test"
        ), "Please specify downstream task: `train`, `test`"
        ...
        self.target = ParticleClouds(
            ...
            data_paths=getattr(config.target.train, "path", None)
            if task == "train"
            else getattr(config.target.test, "path", None),
            **kwargs_target,
        )
        ...
        self.source = ParticleClouds(
            ...
            data_paths=getattr(config.source.train, "path", None)
            if task == "train"
            else getattr(config.source.test, "path", None),
            **kwargs_source,
        )
